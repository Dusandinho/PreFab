...
class Predictor:
    def __init__(self, type, fab, process, version, model_nums):
        ...
        self.models.extend(
            models.load_model(
                f"../models/{type}_{fab}_{process}_{str(version)}_{str(j)}.pb"
            )
    ...
    def predict(self, device, step_length, binary=False):
        # slice image up
        x_slices_4D = np.lib.stride_tricks.sliding_window_view(device, self.slice_size)[
            ::step_length, ::step_length
        ]
        x_slices = x_slices_4D.reshape(-1, *self.slice_size)
        x_slices = tf.reshape(
            x_slices, [len(x_slices), self.slice_length, self.slice_length, 1]
        )
        ...
        for k in range(0, device.shape[0] - self.slice_length + 1, step_length):
            for j in range(0, device.shape[1] - self.slice_length + 1, step_length):
                y[k : k + self.slice_length, j : j + self.slice_length] += y_slices[
                    k // step_length, j // step_length
                ]
                avg_mtx[
                    k : k + self.slice_length, j : j + self.slice_length
